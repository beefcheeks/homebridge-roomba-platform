...
class RoombaDevice {
    ...
    async init() {
        const state = await this.connect.init()
        const {
            hwPartsRev,
            name,
            sku
    ...
    setStateDetails(state) {
        ...
        // Default to structured lastCommand if it does not exist
        ...
        // If there is a new mission ID save it and log it
        if (missionId !== this.missionId) {
            this.missionId = missionId
            this.log(`New mission ID detected: ${missionId}`)
            if (this.phase === RUNNING) {
                // Log room command data to help with initial configuration
                this.log(`Saving room info to missionID map - pMapId: ${this.pMapId}, regionIds: ${this.regionIds}`)
                // Reset mapping to prevent unlimited size creep
                this.missionIdMap = {}
                this.missionIdMap[missionId] = {pMapId: this.pMapId, regionIds: this.regionIds}
            }
        }
    ...
    getCurrentControlStatus() {
        ...
        switch (this.phase) {
            ...
            case EVACUATING:
                ...
                break
            case FINISHING: case RETURNING: case RUNNING:
            ...
            case STUCK:
                ...
                break
            default:
                this.log.error(`Unrecognized current phase: ${this.phase}, defaulting to ${this.Characteristic.CurrentMediaState.STOP}`)
                value =  this.Characteristic.CurrentMediaState.STOP
                break
            }
    ...
    getMissionDetails() {
        ...
        if (this.missionId
            ...
            missionDetails = this.missionIdMap[this.missionId]
        } else {
            missionDetails = {pMapId: null, regionIds: null}
        }
        this.log.debug(`getMissionDetails: ${JSON.stringify(missionDetails)}`)
    ...
    getRoomStatus(name, {pMapId, regionIds}) {
        ...
        if (this.phase === RUNNING) {
            ...
            if (!currentPMapId || !currentRegionIds) {
                const missionDetails = this.getMissionDetails()
                currentPMapId = missionDetails.pMapId
                currentRegionIds = missionDetails.regionIds
            }
            value = (
                pMapId === currentPMapId
             && currentRegionIds
             && currentRegionIds.length === regionIds.length
             && currentRegionIds.every((regionId) => regionIds.indexOf(regionId) >= 0)
            )
        }
