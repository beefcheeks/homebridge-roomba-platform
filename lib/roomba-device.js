...
class RoombaDevice {
    ...
    constructor(config, log, Characteristic) {
        this._config = config
        this.log = log
        // Will log formatted room command data on state sync if true
        this.logRoomCommands = !!this.config.logRoomCommands
        ...
        this.pauseSwitchService.getCharacteristic(this.Characteristic.On)
            ...
            .onSet(async (value) => {
                ...
                if (success) {
                    ...
                    // Only set room switches to off if actually pausing
                    if (value) {
                        // Ensure lastRoomName is preserved when rooms switched off
                        this.updateRoomSwitchesToOff(false)
                    // If resuming a saved room switch, turn that room switch on
        ...
        this.roomSwitchServices[name].getCharacteristic(this.Characteristic.On)
            ...
            .onSet(async () => {
                ...
                if (success) {
                    ...
                    this.updateReturnSwitch(!value)
                    this.updateStartSwitch(!value)
                    // Set lastRoomName for fast resume command
                    if (!value) {
    ...
    updateRoomSwitch(name, value) {
        if (!this.roomSwitchService.hasOwnProperty(name)) {
